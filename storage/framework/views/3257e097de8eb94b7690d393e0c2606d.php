            ...
            <div class="row g-3">
                ...
                <?php if($businessAccount->status->value === \App\Enum\StatusEnum::PENDING->value): ?>
                    <div class="col-12 col-md-6">
                        <form action="<?php echo e(route('business-accounts.update-status', $businessAccount->id)); ?>" method="POST" onsubmit="return confirm('<?php echo e(__("business.approve_confirm_msg")); ?>')">
                            <?php echo csrf_field(); ?>
                ...
                <?php elseif($businessAccount->status->value === \App\Enum\StatusEnum::INACTIVE->value): ?>
                    <div class="col-12">
                        <form action="<?php echo e(route('business-accounts.update-status', $businessAccount->id)); ?>" method="POST" onsubmit="return confirm('<?php echo e(__("business.activate_confirm_msg")); ?>')">
                            <?php echo csrf_field(); ?>
                            <input type="hidden" name="status" value="approved">
                            <button type="submit" class="btn btn-approve w-100 btn-lg rounded-3 py-3 fw-bold shadow-sm">
...
<div class="modal fade" id="deactivateModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content shadow-lg border-0">
            <div class="modal-header border-bottom p-4">
                <h5 class="modal-title text-warning fw-bold fs-4"><?php echo e(__('business.deactivate_acc')); ?></h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="<?php echo e(route('business-accounts.update-status', $businessAccount->id)); ?>" method="POST">
                <?php echo csrf_field(); ?>
                <div class="modal-body p-4">
                    ...
                    <label for="deactivation_reason" class="form-label fw-bold mb-2"><?php echo e(__('business.deactivation_reason')); ?></label>
                    <textarea id="deactivation_reason" name="reason" class="form-control border-2" rows="4"
                        placeholder="<?php echo e(__('business.deactivation_placeholder')); ?>"></textarea>
                </div>
                <div class="modal-footer border-top p-4">
                    <button type="button" class="btn btn-label-secondary px-4 py-2" data-bs-dismiss="modal"><?php echo e(__('business.cancel')); ?></button>
                    <button type="submit" class="btn btn-warning text-white px-4 py-2 fw-bold shadow-sm"><?php echo e(__('business.confirm_deactivation')); ?></button>
                </div>
            </form>
        </div>
    </div>
