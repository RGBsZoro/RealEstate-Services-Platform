...
<style>
    /* تحسين شكل العناصر غير المقروءة */
    .notification-card {
...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
        <div>
            ...
            <p class="text-muted mb-0 small">Manage and track all your system alerts and activities</p>
        </div>
    ...
    <div class="card shadow-sm border-0">
        <div class="card-header border-bottom d-flex align-items-center justify-content-between py-3">
            ...
            {{-- <div class="dropdown">
                <button class="btn p-0" type="button" id="notifActions" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <i class="bx bx-dots-vertical-rounded"></i>
                </button>
                <div class="dropdown-menu dropdown-menu-end" aria-labelledby="notifActions">
                    <a class="dropdown-item" href="javascript:void(0);">Notification Settings</a>
                    <a class="dropdown-item text-danger" href="javascript:void(0);">Clear All History</a>
                </div>
            </div> --}}
        </div>
                    ...
                    <div class="d-flex align-items-center">
                        <div class="flex-shrink-0 me-4">
                            <div class="avatar avatar-md">
                                <span class="avatar-initial rounded-circle {{ $notification->read_at ? 'bg-label-secondary' : 'bg-label-primary' }}">
                                    <i class="bx {{ $notification->data['icon'] ?? 'bx-bell' }} fs-4"></i>
                                </span>
                            </div>
                        </div>
                            ...
                            <a href="{{ route('notifications.readAndRedirect', $notification->id) }}" class="text-decoration-none">
                                ...
                                <div class="d-flex align-items-center gap-3">
                                    <small class="text-muted d-flex align-items-center">
                                        <i class="bx bx-calendar-alt me-1"></i> {{ $notification->created_at->format('M d, Y') }}
                                    </small>
                                    <small class="text-muted d-flex align-items-center">
                        ...
                        <div class="flex-shrink-0 ms-3 d-flex flex-column align-items-center gap-2">
                            <form action="{{ route('notifications.destroy', $notification->id) }}" method="POST" onsubmit="return confirm('Delete this notification?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-icon btn-outline-label-secondary btn-sm border-0 btn-delete-hover" title="Delete">
                                    <i class="bx bx-trash fs-5"></i>
                                </button>
                            </form>
                            @if(!$notification->read_at)
                                <span class="badge bg-label-primary p-1 rounded" title="Unread">
                                    <i class="bx bx-bullseye fs-6"></i>
                                </span>
                            @endif
                        </div>
                    </div>
            ...
            @empty
                <div class="card-body p-5 text-center">
                    ...
                    <p class="text-muted mx-auto" style="max-width: 300px;">
                        When you receive an alert or update, it will appear here in your activity feed.
                    </p>
    ...
    <div class="d-flex justify-content-between align-items-center mt-4 px-2">
        <small class="text-muted">Showing {{ $notifications->firstItem() ?? 0 }} to {{ $notifications->lastItem() ?? 0 }} of {{ $notifications->total() }} entries</small>
        <div>
